...
class MQTTGate2:
    def __init__(self, config, system):
        ...
        self.topics = {
            'entrance': 'xparking/gate2/entrance',
            'exit': 'xparking/gate2/exit',
            'slots': 'xparking/gate2/slots',
            'alert': 'xparking/gate2/alert',
            'command': 'xparking/gate2/command',
        }
    ...
    def connect(self):
        try:
            ...
            logger.error(f"[G2] MQTT error: {e}")
            return False
    
    def _on_connect(self, client, userdata, flags, rc):
        if rc == 0:
            logger.info("[G2] MQTT connected")
            for name, topic in self.topics.items():
                if name != 'command':
                    client.subscribe(topic)
            self.display("in", "X-PARKING", "GATE 2")
            self.display("out", "X-PARKING", "GATE 2")
        ...
        try:
            ...
            if topic == self.topics['entrance']:
                if event == 'CAR_DETECT_IN':
                    ...
                    logger.info("[G2_In] CAR_ENTERED → +1 slot")
                    self.system.executor.submit(self.system._on_car_entered, 2)
            
            elif topic == self.topics['exit']:
                if event == 'CAR_DETECT':
                    logger.info("[G2_Out] detected")
                    ...
                    logger.info("[G2_Out] CAR_EXITED → -1 slot")
                    self.system.executor.submit(self.system._on_car_exited, 2)
            ...
            logger.error(f"[G2] MQTT msg error: {e}")
    ...
    def disconnect(self):
        if self.mqtt:
            ...
            logger.info("[G2] MQTT disconnected")
